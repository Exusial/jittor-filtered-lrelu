...
def code_op_read_file():
    ...
    with open("upfirdn2d.h", 'r', encoding='utf-8') as f:
...
class Upfirdn2d(jt.Function):
    ...
    def execute(self, x, f): 
        ...
        self.save_f = f
        assert f.ndim == 2, "Only support [height, weight] filter for now."
        cuda_src = f'''
            @alias(x_inp, in0)
            @alias(f_inp, in1)
            @alias(y_oup, out0)
            ...
            int downx = {self.downx};
            int downy = {self.downy};
            int upx = {self.upx};
            int upy = {self.upy};
            int padx0 = {self.px0};
            int padx1 = {self.px1};
            ...
            bool flip = {1 if self.flip_filter else 0};
            assert(upx >= 1 && upy >= 1);
        ...
        '''
        outW = (x.shape[3] * self.upx + self.px0 + self.px1 - f.shape[1] + self.downx) // self.downx
        outH = (x.shape[2] * self.upy + self.py0 + self.py1 - f.shape[0] + self.downy) // self.downy
    ...
    def grad(self, grads):  # only support backward for bias and input now (in pytorch.)
        ...
        cuda_src = f'''
            ...
            int pady1 = {p[3]};
            float gain = {self.gain};
            bool flip = {0 if self.flip_filter else 1};
            assert(upx >= 1 && upy >= 1);
            assert(downx >= 1 && downy >= 1);
            ...
            CUDA_CHECK(cudaLaunchKernel(spec.kernel, gridSize, blockSize, args, 0, stream));
        '''
        dx = jt.code(self.x_shape, self.x_dtype, [grads, self.save_f], cuda_header=cuda_header, cuda_src=cuda_src)
        df = None
        return dx, None  

if __name__ == "__main__":
    nx = np.random.uniform(0,1,(1, 1, 3, 3)).astype("float32")
    nf = np.random.uniform(-1,1,(1, 1)).astype("float32")
    frelu = Upfirdn2d()
    x = jt.array(nx)
    f = jt.array(nf)
    out = frelu(x ,f)
